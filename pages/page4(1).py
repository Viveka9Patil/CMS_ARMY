...
st.set_page_config(page_title='CMS Platform',  layout='wide', page_icon='lvlAlpha_logo.png', 
...
with st.container():
    ...
    selectCanvas = st.radio("Body Layer",
                            options = ('Skin', 'Muscular', 'Skeleton', 'Nervous',"Interactive Body"),
                            horizontal = True,
    ...
    if selectCanvas == 'Skin':
        fbody_canvas = st_canvas(
            stroke_width = 3,
            background_image = Image.open("Skin_Body.png"),
            height = 720,
            width = 1050,
            key = "Skin"
        )
        ...
        fbody_canvas = st_canvas(
            stroke_width = 3,
            background_image = Image.open("Muscle_Body.png"),
            height = 720,
            width = 1050,
            key = "Muscular"
        )
        ...
        fbody_canvas = st_canvas(
            stroke_width = 3,
            background_image = Image.open("Skeleton_Body.png"),
            height = 720,
            width = 1050,
            key = "Skeleton"
        )
    elif selectCanvas == 'Nervous':
        fbody_canvas = st_canvas(
            stroke_width = 3,
            background_image = Image.open("Nerve_Body.png"),
            height = 720,
            width = 1050,
            key = "Nerves"
        )
    else:
        selectCanvas=='Interactive Body'
        fbody_canvas = st_canvas(
        #st.title("Interactive Body Parts")

# Embed the HTML code for the interactive buttons
      ...
      <div>
         ...
         <button class="button" id="head">Head</button>
         <button class="button" id="torso">Torso</button>
         <button class="button" id="legs">Legs</button>
      </div>

      <script>
       JavaScript code to handle button clicks and update the selection
      document.getElementById("head").addEventListener("click", function() {
        document.getElementById("selection").innerText = "Head selected";
      });

      document.getElementById("torso").addEventListener("click", function() {
        document.getElementById("selection").innerText = "Torso selected";
       });

     document.getElementById("legs").addEventListener("click", function() {
        document.getElementById("selection").innerText = "Legs selected";
       });
     </script>
...
#st.sidebar.image("E:\lvl alpha\CMS\imgs\IOCL_logo.png", caption=None,  use_column_width=True, clamp=True, channels="RGB", output_format="PNG")
